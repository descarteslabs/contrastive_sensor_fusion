...
from lucid4keras import channel, keras_render_vis, prepare_model
...
def main(_):
    ...
    transforms = [
        ...
        transform.random_rotate(
            list(range(-10, 11)) + list(range(-5, 6)) + list(range(-20, 21, 10))
        ),
        transform.jitter(2),
    ]
    ...
    if not os.path.isfile(weightsfile):
        raise RuntimeError(
            "Run pre_visualize.py first, in a tensorflow 2.x "
    ...
    base_inputs_stack3 = band_fill_layer(inputs_stack3)
    model_stack3 = Model(
        inputs=inputs_stack3, outputs=model_base_stack3(base_inputs_stack3)
    )
    ...
    base_inputs_stack4 = band_fill_layer(inputs_stack4)
    model_stack4 = Model(
        inputs=inputs_stack4, outputs=model_base_stack4(base_inputs_stack4)
    )
